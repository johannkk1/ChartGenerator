...
def get_economic_data():
    ...
    return {
        ...
        'gold': get_gold(),
        'oil': get_oil(),
        'wilshire_5000': get_wilshire_5000(),
...
def get_static_history(series_id):
    ...
    # US Treasury General Account (Weekly) - WDTGAL
    # Real TGA balance: Highly variable based on govt spending/receipts
    # Typical range: $100B - $1.5T
    elif series_id == 'WDTGAL':
        start_date = datetime(2020, 1, 1)  # TGA became important post-2020
        # Generate weekly control points first
        weekly_dates = pd.date_range(start=start_date, end=datetime.now(), freq='W')
        
        values = []
        for d in weekly_dates:
            # TGA fluctuates based on debt ceiling, govt operations
            if d.year == 2020:
                # Covid stimulus drawdown
                if d.month < 3: val = 400
                else: val = 1600 - (d.month-2)*100  # Drew down from $1.6T
            elif d.year == 2021:
                # Continued drawdown + debt ceiling drama
                val = 800 - (d.month/12)*400  # Ended ~$400B
            elif d.year == 2022:
                # Rebuild after debt ceiling
                val = 400 + (d.month/12)*300  # Ended ~$700B
            elif d.year == 2023:
                # Debt ceiling crisis again
                if d.month < 6: val = 700 - (d.month/6)*650  # Drew to $50B
                else: val = 50 + ((d.month-6)/6)*600  # Rebuilt to $650B
            elif d.year >= 2024:
                # Current era - maintain ~$750B
                val = 650 + (d.month/12)*100 + (d.dayofyear % 30) * 5  # Weekly variation
            else:
                val = 400
            values.append(max(50, val))  # Floor at $50B
        
        # Create weekly DataFrame
        df_weekly = pd.DataFrame({'Close': values}, index=weekly_dates)
        
        # Interpolate to daily for smoother charts (linear interpolation, no scipy needed)
        daily_dates = pd.date_range(start=start_date, end=datetime.now(), freq='D')
        df_daily = df_weekly.reindex(daily_dates)
...
def get_wilshire_5000():
    """Get latest Wilshire 5000 Total Market Index"""
    def fetch():
        try:
            # Fetch last 2 observations to calculate change
            data = fetch_fred_data(FRED_SERIES['wilshire_5000'], limit=2)
            if data and len(data) >= 2:
                current = float(data[0]['value'])
                previous = float(data[1]['value'])
                change = current - previous
                ...
                return {
                    ...
                    'unit': '',
                    'label': 'Wilshire 5000',
                    'ticker': FRED_SERIES['wilshire_5000'],
                    'description': 'The Wilshire 5000 Total Market Index is the broadest stock market index of publicly traded American companies, often used as a measure of the total U.S. stock market.',
                    'impact': '📈 A rising index indicates broad equity market strength and economic optimism. A falling index suggests market weakness and potential economic slowdown.'
                }
        except Exception as e:
            print(f"Wilshire 5000 error: {e}")
            
        return None
    
    return _get_cached_or_fetch('wilshire_5000', fetch, ttl=3600)

def get_treasury_general_account():
    """Get latest U.S. Treasury General Account balance"""
    def fetch():
        try:
            # Fetch last 2 observations to calculate change
            data = fetch_fred_data(FRED_SERIES['treasury_general_account'], limit=2)
            if data and len(data) >= 2:
                current = float(data[0]['value'])
                previous = float(data[1]['value'])
                change = current - previous
                ...
                return {
                    'value': f"${current/1000:.1f}B",
                    'change': change,
                    'change_pct': change_pct,
                    'unit': '',
                    'label': 'Treasury General Account',
                    'ticker': FRED_SERIES['treasury_general_account'],
                    'description': 'The U.S. Treasury General Account is the checking account used by the Treasury to handle daily receipts and payments. A declining TGA can inject liquidity into the financial system.',
                    'impact': '💰 A rising TGA draws liquidity from markets (can be bearish for risk assets). A falling TGA injects liquidity (can be bullish). Sharp changes often precede market volatility.'
                }
        except Exception as e:
            print(f"Treasury General Account error: {e}")
            
        return None
    
    return _get_cached_or_fetch('tga', fetch, ttl=3600)
