...
class NewsManager:
    def __init__(self, filepath='saved_news.json'):
        self.filepath = filepath
        self._ensure_file()
    ...
    def save_news(self, news_item):
        saved = self.get_saved_news()
        # Check for duplicates based on title/url
        if any(item.get('title') == news_item.get('title') for item in saved):
            return False # Already saved
        
        saved.insert(0, news_item) # Add to top
        with open(self.filepath, 'w') as f:
    ...
    def delete_news(self, title):
        ...
        saved = [item for item in saved if item.get('title') != title]
